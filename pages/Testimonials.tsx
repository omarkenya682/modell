...
const Testimonials: React.FC<TestimonialsProps> = ({ settings }) => {
  ...
  // Use the video URL from settings, or fallback
  const videoUrl =
    settings?.testimonialVideoUrl ||
    "https://www.youtube.com/embed/dQw4w9WgXcQ?si=placeholder";

  useEffect(() => {
    const loadData = async () => {
      const items = await db.getTestimonials();
      // ✅ Show all testimonials for now
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto mb-16">
          {testimonials.length > 0 ? (
            testimonials.map((item) => (
              <div
                key={item.id}
                className="bg-white p-8 rounded-xl shadow-md border border-gray-100 relative hover:shadow-xl transition-shadow duration-300"
              >
